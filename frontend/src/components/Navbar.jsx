        ...
        <div className='h-full flex flex-col overflow-y-auto text-gray-700 dark:text-gray-300'>
          <div className='flex items-center justify-between p-4 border-b border-blue-100 dark:border-zinc-800 bg-white/70 dark:bg-black/70 rounded-tl-3xl'>
            <button onClick={() => { setVisible(false); navigate('/profile') }} className='flex items-center gap-3 text-left w-full'>
              <div className='bg-white dark:bg-zinc-800 rounded-xl p-2.5 border border-blue-100 dark:border-zinc-700 shadow-sm'>
                <img className='h-5 w-5 invert-0 dark:invert' src={assets.profile_icon} alt='profile' />
              </div>
              <div>
  ...
  return (
    <div className='glass flex items-center justify-between py-4 px-4 sm:px-6 mt-4 relative z-10'>
      ...
      <div className='hidden lg:flex items-center gap-5'>
        <button className='btn-ghost !p-2.5 flex items-center justify-center' aria-label="Search" onClick={() => { setShowSearch(true); navigate('/collection') }}>
          <img src={assets.search_icon} className='w-6 h-6 object-contain shrink-0 invert-0 dark:invert' alt="" />
        </button>

        <button 
          onClick={toggleTheme} 
          className='btn-ghost !p-2.5 flex items-center justify-center transition-all duration-300'
          aria-label="Toggle theme"
        >
          {theme === 'light' ? (
            <svg className="w-6 h-6 text-gray-700" fill="currentColor" viewBox="0 0 20 20"><path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z"></path></svg>
          ) : (
        ...
        <div className='group relative'>
          <button
            className='btn-ghost !p-2.5 flex items-center justify-center'
            aria-label="Profile"
            onClick={() => token ? setIsProfileOpen(v => !v) : navigate('/login')}
          >
            <img
              src={assets.profile_icon}
              className='w-6 h-6 object-contain shrink-0 invert-0 dark:invert'
              alt=""
            />
          </button>
          {/* Dropdown Menu */}
        ...
        <Link to='/cart' className='relative'>
          <button className='btn-primary !p-2.5 flex items-center justify-center' aria-label="Cart">
            <img src={assets.cart_icon} className='w-5 h-5 object-contain invert-0 dark:invert' alt="" />
          </button>
          <p className='absolute -right-1.5 -bottom-1.5 w-5 h-5 flex items-center justify-center bg-gray-900 dark:bg-blue-500 text-white rounded-full text-[10px] font-bold shadow-sm'>{getCartCount()}</p>
        </Link>
      </div>
      <button className='btn-ghost !p-2 lg:hidden text-gray-700 dark:text-gray-300' aria-label="Open menu" onClick={() => setVisible(true)}>
        <img src={assets.menu_icon} className='w-6 h-6 object-contain invert-0 dark:invert' alt="" />
      </button>
